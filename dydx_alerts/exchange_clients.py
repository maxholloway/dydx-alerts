...
class CcxtBaseClient(BaseClient):
    ...
    async def get_index_price(self, market: str) -> float:
        try:
            ...
            price_points = list(
                filter(lambda x: x != None, [data["bid"], data["ask"], data["last"]])
            )
            return median(price_points)
        except Exception as ex:
            print("Exception index price")
            self.logger.error(
                f"Exception occurred when getting index price. Skipping it and returning -1.\n{ex}",
                exc_info=True,
            )
            return -1
...
class OkexClient(ManualClient):
    ...
    def __init__(self):
        super(OkexClient, self).__init__(
            10, 2
        )  # should allow 20 requests / 2 seconds, but the okex api sucks and it over-flags on the rate limit
    ...
    async def get_index_price_no_rate_limit(self, market: str) -> float:
        ...
        index_price = None
        request_url = (
            f"https://www.okex.com/api/v5/market/ticker?instId={base}-{quote}-SWAP"
        )
